...
def self_heal(parameters: dict, player=None) -> str:
    ...
    if action == "scan_all":
        ...
        report = (
            f"=== ESCANEO COMPLETO DE {len(files)} ARCHIVOS ===\n"
            f"Saludables: {healthy_count} | Con errores: {len(files) - healthy_count} | "
            f"Total errores: {total_errors}\n\n"
        )
        for r in results:
            icon = "✅" if r["healthy"] else "❌"
            report += f"{icon} {r['file']} ({r['error_count']} errores)\n"
            for e in r["errors"][:5]:
                report += f"   L{e['line']}: [{e['type']}] {e['message']}\n"
            if len(r["errors"]) > 5:
                report += f"   ... y {len(r['errors']) - 5} más\n"
