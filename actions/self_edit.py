# -*- coding: utf-8 -*-
"""
self_edit.py — Permite a ERIS editar sus propios archivos de código fuente.
...
def _make_backup(file_path: Path) -> str:
    ...
    relative = file_path.relative_to(ERIS_ROOT)
...
def _resolve_path(file_ref: str) -> Path:
    """
    Resuelve una referencia de archivo relativa al proyecto ERIS.
    ...
    if p.is_absolute():
        ...
        try:
            p.relative_to(ERIS_ROOT)
            return p
        except ValueError:
            raise ValueError(
                f"Ruta fuera del proyecto ERIS. Solo se pueden editar archivos dentro de: {ERIS_ROOT}"
            )
    # Relativa al root del proyecto
    resolved = (ERIS_ROOT / p).resolve()
    try:
        resolved.relative_to(ERIS_ROOT)
    except ValueError:
...
def self_edit(parameters: dict, player=None) -> str:
    ...
    # ── RESTORE BACKUP ────────────────────────────────────────────────────
        ...
        original_path = ERIS_ROOT / original_rel
