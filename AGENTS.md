...
Windows desktop assistant (Python 3.14, PyQt6). 493 tools, NeuroSpheres brain, dual Ollama/Gemini chat, Kokoro-82M local Spanish TTS (ef_dora).
...
| `core/pentest_lab.py` | PENTEST LAB (`pentest_lab`): laboratorio de pentesting aislado en VirtualBox. Scope 192.168.56.0/24 exclusivo. Herramientas: nmap (escaneo/vuln con scripts rápidos por puerto), hydra (fuerza bruta), tshark/tcpdump (captura de tráfico), snapshots de VMs. Wrapper VBoxManage para gestionar VMs (listar/iniciar/apagar/snapshots/red interna). Reports en `data/pentest_reports/`. Config en `data/pentest_lab.json`. Nunca toca la red real. Acciones: status/setup/start/stop/reset/snap/restore/scan/vuln/brute/sniff/report/**learn/leccion/escalada**/info/snaps. |
