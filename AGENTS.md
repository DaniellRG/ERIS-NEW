...
Windows desktop assistant (Python 3.14, PyQt6). 491 tools, NeuroSpheres brain, dual Ollama/Gemini chat, Kokoro-82M local Spanish TTS (ef_dora).
...
| `core/agent_definitions.py` | 13 fragmentos (core, web, file, dev, media, comm, vision, security, study, linux, guardian, mentora, memoria). Los 12 previos `handle_*` de `agents/` ahora TODOS se despachan en main.py (antes 8 quedaban muertos); `memoria` es el nuevo. `core/agent_router.py:classify_intent()` umbral ≥3 + penalties + anti-bounce. |
| `core/tool_registry.py` | 491 tool callables |
| `core/tool_declarations.py` | 491 LLM-facing declarations (0 dupes, sync con registry) |
