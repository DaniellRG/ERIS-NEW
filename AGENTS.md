...
| `core/pentest_lab.py` | PENTEST LAB (`pentest_lab`): laboratorio de pentesting aislado en VirtualBox. Scope 192.168.56.0/24 exclusivo. Herramientas: nmap (escaneo/vuln), hydra (fuerza bruta), tshark/tcpdump (captura de tráfico), snapshots de VMs. Wrapper VBoxManage para gestionar VMs (listar/iniciar/apagar/snapshots/red interna). Reports en `data/pentest_reports/`. Config en `data/pentest_lab.json`. Nunca toca la red real. |
