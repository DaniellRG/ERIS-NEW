"""sounds.py — ERIS ambient sound player.

Reproduce 'sonido_eris.mp3' en loop continuo mientras ERIS piensa,
y se detiene en cuanto ERIS empieza a hablar.
...
_MP3_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets", "sonido_eris.MP3")
    ...
    while not stop.is_set():
        ...
        for cmd in _subprocess_players():
            try:
                ...
                print(f"[ERIS Sound] Error ({cmd[0]}): {e}")
                continue
...
def _play_loop(stop: threading.Event) -> None:
    if not os.path.isfile(_MP3_PATH):
        print(
            f"[ERIS Sound] Archivo no encontrado: {_MP3_PATH}\n"
            f"  → Ponelo como 'sonido_eris.mp3' en la misma carpeta que sounds.py"
        )
        return
    ...
    if sys.platform == "win32":
        if _play_loop_windows_mci(stop):
            return
        print("[ERIS Sound] MCI falló, probando players externos...")
    ...
    if not _play_loop_subprocess(stop):
        ...
        print(f"[ERIS Sound] No se encontró player de MP3.\n{msg}")
...
def stop_thinking_sound() -> None:
    """Detiene el sonido inmediatamente (llamar cuando ERIS empieza a hablar)."""
    global _active_stop, _active_thread
    with _lock:
        if _active_stop:
            _active_stop.set()
