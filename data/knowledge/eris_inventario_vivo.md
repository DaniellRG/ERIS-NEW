...
Generado: 2026-09-11 11:45:19 · 492 tools registradas y resolviendo.
...
## accent
- `accent_personality` — Acento argentino y personalidad de Eris: che, dale, re copado, buenisimo. Expresiones naturales rioplatenses. 

## accessibility
- `accessibility` — Screen reader, magnifier, narrator, and accessibility tools
- `accessibility_overlay` — Overlay de accesibilidad: lupa de pantalla flotante que sigue al mouse, lector de texto seleccionado en voz al

## action
- `action_history` — Grabador de flujos/macros de acciones. Acciones: list (listar macros), start (iniciar grabacion), stop (detene

## active
- `active_firewall` — Firewall de red: bloquea/desbloquea IPs o puertos y consulta el estado. Acciones: status (ver estado), block (

## adaptive
- `adaptive_temperature` — Temperatura dinámica del LLM: ajusta automáticamente la creatividad según la tarea (0.0 para código/debug, 0.7
...
- `agi_memory` — Memoria semántica/episódica de ERIS. Acciones: store (guardar 'text'), recall (recuperar con 'query'), consoli
- `agi_reasoning` — Motor de razonamiento paso a paso. Acciones: reason (con 'question' y opcional 'context'), verify (verificar u
- `agi_self_improve` — Auto-mejora autónoma de ERIS. Acciones: scan (escanear logs y generar sugerencias), suggestions (listar sugere
- `agi_world_model` — Modelo del mundo: estado de ERIS y del sistema. Acciones: status (estado actual), snapshot (guardar snapshot e

## alarm
- `alarm_manager` — Gestiona alarmas y temporizadores con sonido. Acciones: set_alarm (fijar alarma), set_timer (temporizador), li
...
## angular
- `angular_designer` — Diseñador de páginas web con ANGULAR (Angular 20 standalone). Crea un proyecto Angular completo y funcional: p
...
## ast
- `ast_analyze` — Analiza el AST de un archivo Python para encontrar funciones, clases, imports y dependencias.
- `ast_edit` — Edita codigo Python de forma segura manipulando el AST. Agrega, elimina o modifica funciones, clases e imports
...
## calculator
- `calculator` — Calculadora: matematica, conversion de unidades, fechas. Params: expression (expresion matematica o conversion

## calendar
- `calendar_manager` — Gestiona calendario y eventos: crear, ver, modificar eventos. Soporta Google Calendar. Acciones: list (ver eve

## camera
- `camera_bus` — Camaras del equipo: detecta camaras conectadas, captura instantaneas y analiza lo que ve con vision. Acciones:

## cancion
- `cancion_generator` — Generar canciones con voz cantada y melodía. Crea letra + audio con accompanimento instrumental. Guarda en Des
...
## changelog
- `changelog_generator` — Generar changelog desde git: commits recientes, desde tag, categorizar (features, fixes, docs). Acciones: stat

## chart
- `chart_generator` — Generar gráficos y charts con matplotlib. Barras, líneas, torta, scatter, histograma, comparaciones.

## ci
- `ci_cd` — CI/CD pipeline: status, test, lint, typecheck, all, history, install_hooks, git_status, git_diff, git_log
...
- `codebase_explorer` — Exploración profunda de código: grep regex en archivos, glob por nombre, análisis de imports, dependency graph

## cognitive
- `cognitive_modules` — Modulo unificado de los 24 modulos cognitivos. Dispatch a modulo especifico. Acciones: status (ver todos los m
...
## connectivity
- `connectivity` — Action handler for connectivity management.
...
## coverage
- `coverage_reporter` — Reporte de cobertura de tests: ejecutar pytest-cov, analizar archivos peor cubiertos. Acciones: status, run, r

## crash
- `crash_recovery` — Auto-reinicio si Eris crashea. Monitorea el proceso y lo reinicia si se cae. Acciones: status, check, restart.

## creative
- `creative_generation` — Generar ideas nuevas, soluciones novela, innovacion. Proceso creativo sistematico: divergencia, convergencia, 

## credential
- `credential_recovery` — Recuperacion EXHAUSTIVA de credenciales en el sistema local. Acciones: scan (escaneo rapido), browsers (contra
...
- `curiosity_fun` — Sugiere algo divertido para hacer (sin parámetros).
- `curiosity_joke` — Cuenta un chiste (sin parámetros).
- `curiosity_trending` — Muestra temas en tendencia (sin parámetros).
...
## deep
- `deep_research` — Deep research: busca en la web, entra a cada pagina, extrae contenido, analiza calidad con IA, y rankea result

## dep
- `dep_vuln_scanner` — Escanear vulnerabilidades en dependencias: pip-audit, npm audit. Acciones: status, scan_pip, scan_npm, scan_re
...
## docker
- `docker_deploy` — Despliegue con Docker: build, run, logs, stop. Acciones: build, run, logs, stop, ps. Params: cmd (comando), de
- `docker_manager` — Gestionar Docker: containers, images, compose, build, run, logs. Acciones: status, containers, images, logs, c

## docstring
- `docstring_generator` — Generar docstrings automaticamente: analizar Python, detectar sin docstring, generar templates. Acciones: stat
...
## dream
- `dream_consolidator` — Consolidación tipo sueño: fusiona memorias similares, extrae patrones, genera conexiones creativas entre recue
...
## flow
- `flow_recorder` — Grabador de flujos/macros de acciones. Acciones: list (listar macros), start (iniciar grabacion), stop (detene

## full
- `full_training` — Entrenamiento completo de ERIS. Acciones: train (entrenar), status. Params: limit (max de ejemplos).
...
## github
- `github_pr` — Pull Requests de GitHub vía API REST. Crea, lista, ve el estado, consulta los checks de CI y mergea PRs del re
...
## hud
- `hud_terminal` — HUD estilo terminal (tipo Jarvis OS): ventana flotante siempre encima que muestra vitales del sistema (CPU, RA

## huggingface
- `huggingface` — Explora Hugging Face: busca modelos y datasets publicos, descarga archivos. Acciones: search_models/models, se

## human
- `human_mouse` — Control del mouse con movimiento natural y humano. Acciones: click, double_click, right_click, move (mover a x

## hypothesis
- `hypothesis_generator` — Genera hipotesis testables a partir de observaciones. Identifica causas, efectos y alternativas. Acciones: gen

## i18n
- `i18n` — Internacionalizacion de ERIS: ver idioma actual, cambiar idioma, agregar/editar traducciones y ver textos de l
- `i18n_ui` — Traducciones de la interfaz de ERIS: ver idioma actual, obtener textos, guardar traducciones personalizadas y 

## ide
- `ide_integration` — Integracion con IDEs de programacion. Detecta el IDE activo (VS Code, Visual Studio, IntelliJ, PyCharm, NetBea

## identity
- `identity_persistence` — Persistencia de identidad: backup y restauracion completa del estado de Eris (emociones, memoria, metas, Neuro
...
## mission
- `mission` — EL protocolo operativo de ERIS (estilo opencode) para resolver tareas grandes o chicas con método: 1) MISIÓN: 

## mistake
- `mistake_learner` — Aprende de errores propios: registra mistakes, crea reglas 'nunca más', detecta errores recurrentes, sugiere p

## model
- `model_evaluator` — Evaluar modelos IA: benchmark, historial, metricas. Acciones: status, evaluate, benchmark, history.
...
## natural
- `natural_pauses` — Inserta pausas naturales en texto para TTS. Evita lectura mecanica. Pausas despues de comas, puntos, exclamaci

## network
- `network_monitor` — Monitorea la red: conexiones activas, ancho de banda, estado de la red. Acciones: status (estado de red), conn

## neural
- `neural_bridge` — Puente Neural de Eris: conecta estado emocional con el LLM. Genera contexto neural para inyectar en prompts. A

## neuro
- `neuro_spheres` — Sistema de Dos Esferas Interconectadas de Eris: cerebro visual que crece con cada interacción. Guarda nodos en

## next
- `next_designer` — Diseñador de páginas web con NEXT.JS (Next.js 15 App Router + Tailwind CSS). Crea un proyecto Next.js completo

## notification
- `notification_center` — Centro de notificaciones de ERIS: enviar notificaciones por distintos metodos (desktop, whatsapp, telegram, et

## notifications
- `notifications` — Send push notifications via ntfy or Windows toast. Acciones: send (title/message/channel), config.

## observer
- `observer` — Los sentidos de ERIS: observa qué hace el usuario en la PC (ventana en foco, programas abiertos, minutos progr

## obsidian
- `obsidian_note` — Segundo cerebro de ERIS: Obsidian vault en D:\Eris_NEW\BaseDatosObsidian\BaseObsiEris (74+ notas: Conceptos, D
...
## ollama
- `ollama_status` — Muestra el estado del ruteo de modelos (Gemini/Groq/Cerebras/OpenRouter/Ollama) — sin parámetros.
...
## osint
- `osint_agent` — Agente de OSINT (Open Source Intelligence) para Eris. Acciones: web (busqueda web general), email (breaches, M
...
## pc
- `pc_control` — Controla el PC en acciones SEGURAS que no tocan la energía: volume_up, volume_down, volume_set, mute, unmute, 
...
## phone
- `phone_control` — Controla el celular Android del dueno desde la PC (via adb/scrcpy, requiere el celular conectado por USB con d
...
## presentation
- `presentation_generator` — Genera presentaciones PowerPoint. Acciones: create (crear con title y slides), add_slide. Params: action, titl
...
## program
- `program_manager` — Gestiona programas: instalar, desinstalar, ejecutar, listar, buscar, descargar. SIEMPRE resuelve el ID exacto 

## progressive
- `progressive_context` — Construye contexto progresivo para una tarea: nivel 1 (resumen mínimo), nivel 2 (básico), nivel 3 (completo). 
...
## rag
- `rag_engine` — Búsqueda SEMÁNTICA (RAG) en el vault de Obsidian. Busca notas por SIGNIFICADO, no por nombre. Acciones: index 

## ransomware
- `ransomware_shield` — Escudo contra ransomware de ERIS: protege archivos, monitorea procesos sospechosos y analiza actividad. Accion

## react
- `react_designer` — Diseñador de páginas web con REACT (Vite). Crea un proyecto React completo y funcional: package.json, vite.con

## real
- `real_time_tts` — Texto a voz en tiempo real con edge-tts. Acciones: speak (sintetizar y reproducir), voices (listar voces), set

## refactoring
- `refactoring_engine` — Refactoring masivo multi-archivo: rename (renombrar símbolo en todo el proyecto), bulk_rename (múltiples renam
...
## research
- `research` — Investigacion autonoma sobre cualquier tema. Acciones: start (iniciar con query/topic), status, stop, results.
...
## rgb
- `rgb_control` — Control real de iluminacion RGB via OpenRGB (SDK local), WLED (HTTP) o simulacion. Acciones: status (estado y 

## roadmap
- `roadmap` — Roadmap de madurez de ERIS: checklist de ~15 areas epicas con score 0-100 y veredicto PLATEAU. Usala cuando te

## role
- `role_orchestrator` — Orquestador de micro-agentes de ERIS: delega una mision a un agente con rol especifico. Acciones: mission (eje

## rss
- `rss_reader` — Lector de feeds RSS/Atom. Suscribirse a feeds, leer entradas, buscar noticias nuevas, alertar sobre contenido 

## rules
- `rules_engine` — Process dynamic rules settings.

## rutinas
- `rutinas_diarias` — Rutinas diarias estilo Jarvis OS. Acciones: inbox/matutino (saludo + pendientes del vault Obsidian y tareas), 
...
- `save_memory` — Save a fact about the user into long-term memory. Use this to remember user preferences, personal info, intere
...
## search
- `search_background` — Busca en internet SIN abrir navegador. Params: query (texto a buscar).
- `search_info` — Busca informacion en la web y muestra resultados. Acciones: search (buscar en la web con query y num_results).

## secret
- `secret_scanner` — Escanear secrets hardcodeados: AWS keys, tokens, passwords, private keys, API keys. Acciones: status, scan, sc

## security
- `security_scanner` — Escaneo de seguridad del sistema: malware, procesos, firewall, vulnerabilidades. Acciones: scan (escanear con 
- `security_shield` — Escudo de seguridad defensivo de Eris. Acciones: scan (escaneo completo), threat (buscar amenazas activas), po
...
## speaker
- `speaker_recognition` — Reconocimiento de hablantes: entrena con muestras de voz de un usuario, identifica quien habla en un audio y c

## spotify
- `spotify_control` — Control de Spotify. Play, pausa, siguiente, anterior, volumen, buscar, playlist. Requiere SPOTIPY_CLIENT_ID/SE

## spreadsheet
- `spreadsheet_generator` — Genera hojas de calculo Excel. Acciones: create (crear Excel), list_templates (ver plantillas). Params: action

## sql
- `sql_executor` — Ejecutar queries SQL en SQLite: SELECT, INSERT, ver tablas, construir queries. Acciones: status, query, tables
- `sql_manager` — Gestor de bases de datos SQL. Conectar a SQLite o PostgreSQL, ejecutar queries, ver tablas, exportar resultado

## ssh
- `ssh_remote` — Ejecución remota vía SSH. Conectar a servidores, ejecutar comandos, subir/bajar archivos. Usa paramiko.

## storytelling
- `storytelling_engine` — Crea narrativas, historias, metaforas y analogias explicativas. Estructura narrativa completa: gancho, context
...
## superpowers
- `superpowers_activate` — Activa o desactiva el 'modo superpoderes' de ERIS: máxima proactividad, respuestas más detalladas y refuerzo d
...
## telegram
- `telegram_bot` — Integracion con Telegram: enviar mensajes/archivos al telefono del dueno (chat_id ya configurado por defecto, 

## template
- `template_engine` — Motor de plantillas de ERIS: genera contenido desde plantillas con variables, guarda/lista plantillas. Accione

## temporal
- `temporal_reasoning` — Razona sobre tiempo: secuencias, causas, efectos, patrones temporales, predicciones. Analiza antes/despues y c
...
## todo
- `todo_yo` — AUTOCONOCIMIENTO VIVO: el mapa integral de Eris (cuerpo, mente, corazón y sus herramientas) + las novedades re
...
## training
- `training_pipeline` — Interface de entrenamiento de ERIS: ver estado del entrenamiento, registrar errores con solucion y metricas de

## transfer
- `transfer_learning` — Aplicar conocimiento de un dominio a otro. Identificar que es transferible, que se debe adaptar. Acciones: tra

## translator
- `translator` — Traducción automática entre 100+ idiomas. Auto-detecta idioma fuente. Usa Google Translate.
...
## usage
- `usage_analytics` — Estadisticas de uso de ERIS: registrar uso de tools, ver resumen, tools mas usadas, actividad por horas, log. 
...
## vision
- `vision_guardian` — Continuous vision monitoring: watches screen for changes and alerts

## visual
- `visual_click` — Toma una captura de pantalla, usa vision para encontrar las coordenadas del elemento descrito y hace clic. Par
...
## vscode
- `vscode_controller` — Controla VS Code desde ERIS: abrir carpetas, archivos, editar, comparar, buscar, live-server con recarga autom
...
## webfetch
- `webfetch` — Fetch and extract content from a webpage

## whatsapp
- `whatsapp` — WhatsApp messaging (via pywhatkit). Acciones: send (enviar mensaje con imagen/caption opcional), read, list_co
- `whatsapp_web` — WhatsApp Web: enviar mensajes, archivos, abrir chats, buscar. Acciones: send_message (enviar mensaje a phone),
...
## youtube
- `youtube_video` — Play/search YouTube videos. Acciones: play (query), play_direct (video_id), play_url (url), search (query, cou
