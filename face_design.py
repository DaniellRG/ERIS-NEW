...
try:
    from face_sounds import play as _sfx_play
except Exception:
    def _sfx_play(name):
        pass
...
def _speech_osc(t):
    """Envoltura tipo habla con ritmo natural: sílabas, consonantes,
    frases (más fuerte / más suave) y micro-pausas donde la boca se cierra."""
    syl = 0.5 + 0.5 * math.sin(t * 19.0 + math.sin(t * 3.0) * 1.4)
    con = 0.5 + 0.5 * math.sin(t * 43.0 + 1.3)
    v = 0.66 * syl + 0.34 * con
    phr = 0.55 + 0.45 * math.sin(t * 2.35 + 0.6)
    v = v * (0.55 + 0.45 * phr)
    g = math.sin(t * 6.98 + 2.0)
    if g > 0.90:
        v = max(v * 0.3, 0.10)
    return max(0.08, min(1.0, v))


# ── Emociones del texto que habla ERIS ───────────────────────────────────
_EMOTION_RULES = (
    (("jaja", "jeje", "jiji", "risa", "jajajaj", "jajaja"), "happy", "laugh"),
    (("te quiero", "te amo", "te adoro", "mi amor", "cariño", "corazón",
      "te extraño", "enamorad", "te quier"), "in_love", None),
    (("qué sorpresa", "no puede ser", "wow", "asombro", "sorprend"), "astonished", None),
    (("sorprendentemente", "increíble", "increible"), "astonished", None),
    (("me alegra", "feliz", "alegría", "alegria", "maravillos", "excelente", "genial",
      "fantástic", "fantastic", "perfecto", "encantad", "content"), "happy", None),
    (("qué linda", "hermos", "precios", "bella", "bonita", "adorable", "encantadora",
      "querida"), "loved", None),
    (("triste", "lo siento", "lamento", "decepcion", "fracaso", "perdón", "perdon",
      "disculpa", "qué pena", "que pena", "tristeza", "dolor"), "holding_tears", None),
    (("enoja", "enfada", "furio", "molest", "irritad", "odio", "frustra",
      "fastidia", "rabia", "basta", "molesta"), "angry", None),
    (("miedo", "asust", "temor", "peligro", "aterrad", "terror", "preocup"), "fearful", None),
    (("sueño", "sueno", "cansad", "agotad"), "sleepy", None),
    (("calor", "caliente", "bochorno", "sofocad"), "hot", None),
    (("frío", "frio", "congel", "temblar", "helado"), "cold", None),
    (("fiesta", "cumpleaños", "celebra", "felicidades", "felicitacion"), "party", None),
    (("depende", "veamos", "pienso", "quizás", "quizas", "tal vez", "a ver",
      "déjame pensar", "dejame pensar", "analiz", "investiguemos", "recordar", "creo que"), "thinking", None),
    (("broma", "guiño", "guino", "coquete", "bromeando", "juego"), "wink", None),
    (("chévere", "chevere", "mola", "cool", "padrísimo", "padrisimo", "bacán", "bacano"), "cool", None),
    (("hambre", "rico", "delicioso", "comida", "yum", "sabros"), "yum", None),
    (("asco", "repugn", "náusea", "nausea", "nauseabund"), "nausea", None),
    (("vamos", "adelante", "dale", "listo", "bien", "bueno", "ok", "vale",
      "perfecto", "claro", "por supuesto", "gracias"), "smiling", None),
)


def _emotion_for(text: str):
    """Busca palabras de sentimiento en el texto y devuelve (expresión, reacción)
    o None si no hay ninguna señal emocional."""
    low = text.lower()
    for words, expr, reaction in _EMOTION_RULES:
        for w in words:
            if w in low:
                return expr, reaction
    return None
...
class FaceWidget(QWidget):
    ...
    def __init__(self, expr="neutral", autostart=True):
        ...
        self.gesture = None
        self.gesture_t = 0.0
        self.gesture_dur = 0.9
        self.next_gesture = random.uniform(16, 38)
    ...
    def express_emotion(self, text, force=False):
        if self.music > 0:
            return
        res = _emotion_for(text or "")
        if res is None:
            return
        expr, reaction = res
        if reaction and self.reaction is None:
            self.set_expr(expr)
            self.trigger_reaction(reaction, silent=True)
        elif expr != self.expr or force:
            self.set_expr(expr)

    def show_expression(self, name, text=""):
        name = (name or "neutral").strip().lower()
        if name in ("laugh", "risa"):
            self.set_expr("grinning")
            self.trigger_reaction("laugh", silent=True)
            return
        aliases = {"sad": "pouting", "surprised": "astonished", "devious": "devil",
                   "confused": "hmm", "smile": "smiling", "rolling": "rolling_eyes",
                   "flirty": "wink", "excited": "happy", "in love": "in_love",
                   "inlove": "in_love", "love": "loved", "blushing": "blush_smile"}
        name = aliases.get(name, name)
        if name not in _EXPRESSIONS:
            name = "neutral"
        self.set_expr(name)

    def trigger_reaction(self, name=None, silent=False):
        if self.reaction is not None:
            return
        self.reaction = name or random.choice(
            ("sigh", "yawn", "flinch", "laugh", "cry", "sneeze", "hiccup", "cough", "deep_sigh"))
        self.reaction_dur = {"sigh": 1.6, "yawn": 2.2, "flinch": 0.7,
                             "laugh": 1.8, "cry": 2.6, "sneeze": 1.0,
                             "hiccup": 0.9, "cough": 0.5, "deep_sigh": 1.8}[self.reaction]
        self.reaction_t = 0.0
        if not silent and self.reaction not in ("sigh", "deep_sigh"):
            _sfx_play(self.reaction)
    ...
    def _effective(self):
        ...
        for k in _SNAP:
            prm[k] = self.cur[k]
        prm["mouth_open"] = max(0.0, min(1.0, prm["mouth_open"]))
        if self.speaking and self.audio > 0.03:
            amp = min(1.0, self.audio * 1.25)
            talk = _speech_osc(self.clock) * amp
            prm["mouth_open"] = prm["mouth_open"] * 0.25 + 0.95 * talk
            prm["mouth_curve"] = prm["mouth_curve"] * 0.8
        ...
        if self.reaction:
            ...
            if self.reaction == "flinch":
                ...
                prm["brow_l"] += 0.4 * env
                prm["brow_r"] += 0.4 * env
            elif self.reaction == "sneeze":
                prm["eye_open_l"] = min(prm["eye_open_l"], 1 - 1.2 * env)
                prm["eye_open_r"] = min(prm["eye_open_r"], 1 - 1.2 * env)
                prm["mouth_open"] = max(prm["mouth_open"], 0.95 * env)
                prm["brow_l"] += 0.6 * env
                prm["brow_r"] += 0.6 * env
            elif self.reaction == "hiccup":
                pulse = 0.5 + 0.5 * math.sin(self.clock * 30)
                prm["eye_open_l"] = max(prm["eye_open_l"], 0.6 + 0.5 * pulse * env)
                prm["eye_open_r"] = max(prm["eye_open_r"], 0.6 + 0.5 * pulse * env)
                prm["mouth_open"] = max(prm["mouth_open"], 0.3 + 0.3 * pulse * env)
            elif self.reaction == "cough":
                prm["mouth_open"] = max(prm["mouth_open"], 0.4 + 0.45 * abs(math.sin(self.clock * 30)) * env)
                prm["eye_open_l"] = min(prm["eye_open_l"], 1 - 0.5 * env)
                prm["eye_open_r"] = min(prm["eye_open_r"], 1 - 0.5 * env)
                prm["brow_l"] += 0.5 * env
                prm["brow_r"] += 0.5 * env
            elif self.reaction == "deep_sigh":
                prm["mouth_open"] = max(prm["mouth_open"], 0.3 + 0.5 * env)
                prm["mouth_curve"] = min(prm["mouth_curve"], -0.2 * env)
                prm["eye_open_l"] = min(prm["eye_open_l"], 1 - 0.6 * env)
                prm["eye_open_r"] = min(prm["eye_open_r"], 1 - 0.6 * env)
        if self.gesture:
            ge = math.sin(math.pi * min(1.0, self.gesture_t / self.gesture_dur))
            if self.gesture == "brow":
                prm["brow_lift"] += 0.45 * ge
                prm["brow_l"] -= 0.25 * ge
            elif self.gesture == "cheek":
                prm["blush"] = max(prm["blush"], 0.3 * ge)
                prm["eye_arc"] = max(prm["eye_arc"], 0.25 * ge)
    ...
    def _tick(self):
        ...
        if self.glitch > 0:
            self.glitch -= dt
        self.next_gesture -= dt
        if self.next_gesture <= 0 and self.gesture is None and self.reaction is None and self.music <= 0:
            self.gesture = random.choice(("brow", "cheek"))
            self.gesture_dur = 0.9
            self.gesture_t = 0.0
            self.next_gesture = random.uniform(20, 45)
        if self.gesture:
            self.gesture_t += dt
            if self.gesture_t >= self.gesture_dur:
                self.gesture = None
        self.next_blink -= dt
        ...
        if self.blink_phase is not None:
            self.blink_phase += dt
            k = min(1.0, self.blink_phase / 0.26)
            if k < 0.4:
                self.blink = math.sin(math.pi * k / 0.4)
            else:
                self.blink = math.sin(math.pi * (1.0 - k) / 0.6)
            if k >= 1.0:
                self.blink_phase = None
                self.blink = 0.0
        self.look_t -= dt
    ...
    def render_to(self, p, rect):
        bob = (math.sin(self.clock * 1.1) * 1.2 + math.sin(self.clock * 2.9) * 2.4 * self.music
               + math.sin(self.clock * 2.3) * 1.6 * self.audio)
        sway = 1.2 + 1.7 * self.music
        prm = self._effective()
        eo = self.eye_offs
        if self.reaction in ("sneeze", "hiccup", "cough"):
            renv = math.sin(math.pi * min(1.0, self.reaction_t / self.reaction_dur))
            jx = math.sin(self.clock * 42) * 3.5 * renv
            jy = math.sin(self.clock * 50) * 2.0 * renv
            eo = ((eo[0][0] + jx, eo[0][1] + jy), (eo[1][0] - jx, eo[1][1] + jy))
        if self.glitch > 0:
        ...
        ga = 20 + int(22 * self.audio) + int(10 * self.music)
        gc = QColor(_BLUE)
        gc.setAlpha(ga)
        glow.setColorAt(0, gc)
        gc0 = QColor(_BLUE)
        gc0.setAlpha(0)
        glow.setColorAt(1, gc0)
        p.save()
        p.setPen(Qt.PenStyle.NoPen)
